...
//              - With that, the data object will contain an error prop which can be "if" checked for a truthy value. If it returns 'true', the error message stored on the data object can be rendered to the browser via the component. With that, the error generation and handling code is handled by the loader function where it belongs, arguably. This is one way of handling errors - simply returning a JS object with an error and message prop that indicates an error has occurred and then using that data as necessary within the component.
//      - Alternatively, the error handling portion of the loader function could instead 'throw' an error instead of returning a JS object with an error and message prop. For this, 'throw' (create) a 'new' error object with the built-in 'Error' constructor, or 'throw' any other kind of object as an error, i.e. throw new Error("Error message...") or throw {message: "Error message..."}. With the second option of simply 'throwing' a JS object - instead of an error - include a message prop with an appropriate error message string as the value. Thus, when an error gets 'thrown' in a loader function, React Router will simply render the closest 'errorElement' located in the router route defintions. This can be applied to any route but it is typically the 'root' route. Therefore is an error occurs or a path that does not exists is navigated to it will display the 'errorElement' instead.
...
//              - NOTE: The 'errorElement' is not just there to show a fallback page in case of invalid route paths. That is one use case but not the only one. The 'errorElement' will be shown on the browser whenever an error is generated in any route related code, including loader functions.
//              - The errorElement prop's value should be the errorElement component. Thus, the errorElement component will be displayed whenever any kind of error that occurs anywhere in the routes occurrs. This even applies to deeply nested routes because errors will 'bubble up'. That is why the errorElement can be placed on the 'root' route, because all erros that occur within a route will bubble up until it reaches that route.

// Lesson 469. Extracting Error Data & Throwing Responses

// For example, we might want to differentiate between 4 0 4 errors and other errors like the one we have here from our loader, where we actually have an error message that we might wanna display instead of the default error message I defined here. So to differentiate between errors what we can do is instead of throwing a object, we can throw a response by again creating a new response. And then we can include some data into that response. For this, we have to call JSON stringify if we want to pass an object to the response. And then we could add a message prop and say could not fetch events. And now we can add this second argument to the response constructor and set the status, for example, to 500 to indicate that something went wrong on the back end. That's just one example. In this case, it's an incorrect URL, but we could set this status code. Now I'm doing this because you can actually get hold of the data that's being thrown as an error inside of the component that's being rendered as an error element. And for that React-Router-Dom gives you another special hook which we import from react-router-dom. And that's the use route error hook. This gives us an error object if you want to call it like this. And the shape of that object now depends on whether you threw a response or any other kind of object or data. If you threw a response as I'm doing it here now, this error object will include a status field. which actually reflects the status of the response you threw. In my case, that would be 500 because that's the status I set here. If you threw any other kind of object, like a regular JavaScript object, then this error object would already be that thrown object. So then there would not be this special status property. But that's why you might wanna throw responses instead of regular objects because it does allow you to include this extra status property, this extra status field, which helps with building a generic error handling component. Because now an error JS in this error page, we can create our title and our message and set these two default values, but override them with more fitting values based on which error we have. So we could, for example, have these default values here, but then we can check if error dot status is maybe 500, in which case we might want to keep the title. But set the message to error.data.message. Now error.data gives us access to the data that's included in this error response that was thrown. So, to this object here, in my case, And that object has a message and we can assume that most objects that are included in error responses will have message properties. That's why I'm accessing the data object of the error response and then the message property here. But we could, for example, also check if the error status is maybe 4 0 4, which is the default status set by React router if you enter a path that's not supported. And in that case, we could set the title to not found, and the message to could not find resource or page. And now we can use these values down here and set the title to our title, which we set conditionally and also output our message here, which is set to different values based on different status codes. This data object here, actually, first of all must be converted back to an object because otherwise it's still JSON, in JSON format. So we must use JSON Parse here, and then access message on the parse data. But with that done, you see I got this error output in case of my error due to me visiting events. And that's why we might want to throw responses in the places where things go wrong and add such a generic error handling page which is rendered with help of an error element added to the root route. That's one way of handling errors and embracing those features that are built into React router.
